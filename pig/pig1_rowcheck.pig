...
total_count = FOREACH (GROUP data ALL) GENERATE 'Raw Row Count' AS label, COUNT(data) AS row_count;
clean_count = FOREACH (GROUP clean ALL) GENERATE 'Cleaned Row Count' AS label, COUNT(clean) AS row_count;

combined = UNION total_count, clean_count;

STORE combined INTO '/ecommerce/output/pig/analysis1' USING PigStorage(',');