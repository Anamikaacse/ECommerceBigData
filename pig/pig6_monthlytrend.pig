...
grouped = GROUP clean BY order_month;
monthly_trend = FOREACH grouped GENERATE group AS month, COUNT(clean) AS order_count, SUM(clean.total_price_usd) AS total_sales;
monthly_sorted = ORDER monthly_trend BY month ASC;
STORE monthly_sorted INTO '/ecommerce/output/pig/MonthlySalesTrend2' USING PigStorage(',');