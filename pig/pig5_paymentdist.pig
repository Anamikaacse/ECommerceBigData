...
grouped = GROUP clean BY payment_method;
pm_summary = FOREACH grouped GENERATE group AS payment_method, COUNT(clean) AS order_count, SUM(clean.total_price_usd) AS total_revenue;
STORE pm_summary INTO '/ecommerce/output/pig/PaymentMethodDistribution' USING PigStorage(',');