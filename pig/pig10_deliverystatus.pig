...
grouped = GROUP clean BY delivery_status;
delivery_summary = FOREACH grouped GENERATE group AS delivery_status, COUNT(clean) AS order_count, AVG(clean.rating) AS avg_rating;
STORE delivery_summary INTO '/ecommerce/output/pig/DeliveryStatusBreakdown' USING PigStorage(',');