...
grouped = GROUP clean BY brand;
brand_profit = FOREACH grouped GENERATE group AS brand, SUM(clean.profit_usd) AS total_profit;
brand_sorted = ORDER brand_profit BY total_profit DESC;
STORE brand_sorted INTO '/ecommerce/output/pig/BrandWiseTotalProfit' USING PigStorage(',');